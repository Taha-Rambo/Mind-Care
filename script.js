(() => {
  ...
  // ✅✅✅ FIXED: Download report as a nice HTML file (not Notepad txt)
...
<style>
  body{font-family: Arial, sans-serif;background:#f7fafc;padding:24px;color:#1a202c;direction:rtl}
  .card{max-width:760px;margin:0 auto;background:#fff;border:1px solid #e2e8f0;border-radius:14px;padding:22px;box-shadow:0 12px 26px rgba(0,0,0,.08)}
  .top{display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;margin-bottom:10px}
  h1{font-size:22px;margin:0}
  .badge{display:inline-block;padding:6px 10px;border-radius:999px;background:#edf2f7;font-size:13px;color:#2d3748}
  .score{font-size:18px;margin:14px 0 6px}
  .meta{font-size:13px;color:#4a5568;margin:0 0 14px}
  .section{margin-top:16px;padding-top:16px;border-top:1px solid #edf2f7}
  .section h2{font-size:16px;margin:0 0 10px}
  ul{margin:0;padding:0 18px 0 0}
  li{margin:6px 0}
  .warn{margin-top:14px;background:#fed7d7;border:1px solid #fc8181;padding:12px;border-radius:10px;color:#742a2a}
  .note{margin-top:14px;font-size:13px;color:#4a5568}
</style>
</head>
<body>
  <div class="card">
    <div class="top">
      <h1>🧠 تقرير Mind Care</h1>
      <span class="badge">${escapeHtml(result.title)}</span>
    </div>

    <p class="meta"><b>التاريخ:</b> ${escapeHtml(date)}</p>
    <p class="score"><b>النتيجة الكلية:</b> ${total}</p>

    <div class="section">
      <h2>ملخص النتيجة</h2>
      <p><b>${escapeHtml(result.subtitle)}</b></p>
      <p>${escapeHtml(result.description)}</p>
    </div>

    <div class="section">
      <h2>التوصيات</h2>
      <ul>${recs || "<li>لا توجد توصيات.</li>"}</ul>
    </div>

    <div class="section">
      <h2>هل تحتاج لاستشارة مختص؟</h2>
      <p>${escapeHtml(result.doctorAdvice)}</p>
      ${urgent ? `<div class="warn"><b>تنبيه:</b> إذا كانت لديك أفكار لإيذاء النفس أو الآخرين الآن، اطلب مساعدة فورية من شخص موثوق أو تواصل مع جهة طوارئ.</div>` : ""}
    </div>

    <div class="note">
      هذا التقرير استرشادي فقط ولا يُعد تشخيصًا طبيًا.
    </div>
  </div>
</body>
</html>`;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (missing.length) console.error("Missing HTML elements (IDs):", missing);
